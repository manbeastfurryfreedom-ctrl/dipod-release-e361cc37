...
from fpo_rsl_rl.utils import resolve_nn_activation
...
class ActorCritic(nn.Module):
    ...
    def __init__(
        ...
        zero_noise_sampling: bool = False,
        activation="elu",
        init_noise_std=1.0,
        actor_scale: float = 1.0,
        actor_mlp_output_scale: float = 1.0,
        actor_final_layer_weight_scale: float = 1.0,
        cfm_loss_t_inverse_cdf_beta: float = 1.0,
        cfm_loss_use_huber: bool = False,
        cfm_loss_huber_delta: float = 1.0,
        cfm_loss_reduction: Literal["mean", "sum", "sqrt"] = "mean",
        action_perturb_std: float = 0.0,
        use_discrete_timesteps_for_training: bool = False,
        **kwargs,
    ):
        if kwargs:
            print(
        ...
        self.mlp_output_scale = actor_mlp_output_scale
        self.cfm_loss_t_inverse_cdf_beta = cfm_loss_t_inverse_cdf_beta
        self.sampling_steps = sampling_steps
        self.cfm_loss_use_huber = cfm_loss_use_huber
        self.cfm_loss_huber_delta = cfm_loss_huber_delta
        ...
        self.use_discrete_timesteps_for_training = use_discrete_timesteps_for_training
        if training_sampling_steps is not None:
            self.training_sampling_steps = training_sampling_steps
            self.use_discrete_timesteps_for_training = True
        else:
            self.training_sampling_steps = sampling_steps
        ...
        if (
            ...
            with torch.no_grad():
                final_layer.weight.data *= actor_final_layer_weight_scale
                if final_layer.bias is not None:
                    final_layer.bias.data *= actor_final_layer_weight_scale
            print(
                f"Applied actor_final_layer_weight_scale={actor_final_layer_weight_scale} to final layer"
            )
    ...
    def act(self, observations: torch.Tensor, **kwargs):
        ...
        def _flow_schedule(flow_steps: int) -> tuple[torch.Tensor, torch.Tensor]:
            if self.use_discrete_timesteps_for_training:
                assert flow_steps <= self.training_sampling_steps, (
                    "Cannot use more steps than trained on"
                )
                full_t_path = torch.linspace(
                    1.0, 0.0, self.training_sampling_steps + 1, device=device
                )
                full_t_path = full_t_path[
                    torch.linspace(
                        0,
                        self.training_sampling_steps,
                        flow_steps + 1,
                        device=device,
                    )
                    .round()
        ...
        t_current, t_next = _flow_schedule(flow_steps)
        dt = t_next - t_current
    ...
    def _compute_squared_error(
        ...
        if self.cfm_loss_use_huber:
            ...
            huber_loss = torch.where(
                abs_diff <= self.cfm_loss_huber_delta,
                diff**2,
                2 * self.cfm_loss_huber_delta * abs_diff - self.cfm_loss_huber_delta**2,
            )
            if self.cfm_loss_reduction == "mean":
                return torch.mean(huber_loss, dim=-1)
            elif self.cfm_loss_reduction == "sum":
                return torch.sum(huber_loss, dim=-1)
            else:  # "sqrt"
                return torch.sum(huber_loss, dim=-1) / (huber_loss.shape[-1] ** 0.5)
        else:
            if self.cfm_loss_reduction == "mean":
