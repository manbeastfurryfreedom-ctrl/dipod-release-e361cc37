...
import fpo_rsl_rl
from fpo_rsl_rl.algorithms import FPO
from fpo_rsl_rl.env import VecEnv
from fpo_rsl_rl.modules import (
    ActorCritic,
    EmpiricalNormalization,
)
from fpo_rsl_rl.utils import store_code_state
...
class OnPolicyRunner:
    ...
    def __init__(
        ...
        self.randomize_reset_episode_progress = self.cfg.get("randomize_reset_episode_progress", 0.0)
        ...
        self.git_status_repos = [fpo_rsl_rl.__file__]

    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        # Run self-distillation if configured
        sd_cfg = self.cfg.get("self_distillation", {})
        if isinstance(sd_cfg, dict) and sd_cfg.get("enabled", False):
            self._run_self_distillation(sd_cfg)
        ...
        if self.log_dir is not None and self.writer is None and not self.disable_logs:
            ...
            if self.logger_type == "neptune":
                from fpo_rsl_rl.utils.neptune_utils import NeptuneSummaryWriter
                ...
                from fpo_rsl_rl.utils.wandb_utils import WandbSummaryWriter
            ...
            if self.logger_type == "wandb" and self.enable_post_training_eval:
                ...
                eval_modes = self.cfg.get("flow_eval_modes", ["zero", "fixed_seed", "random"])
    ...
    def evaluate_checkpoint(self, checkpoint_path, iteration):
        ...
        try:
            ...
            eval_modes = self.cfg.get("flow_eval_modes", ["zero", "fixed_seed", "random"])
            eval_fixed_seed = self.cfg.get("flow_eval_fixed_seed", 12345)
    ...
    def run_post_training_checkpoint_eval(self):
        ...
        print(f"  Eval modes: {self.cfg.get('flow_eval_modes', ['zero', 'fixed_seed', 'random'])}")
    ...
    def _run_self_distillation(self, sd_cfg: dict):
        """Run the self-distillation pre-training phase."""
        from fpo_rsl_rl.runners.self_distillation import run_self_distillation

        n_samples = sd_cfg.get("n_samples_per_action", None)
        if n_samples is None:
            n_samples = self.cfg.get("algorithm", {}).get("n_samples_per_action", 16)

        print("\n" + "=" * 80)
        print("Starting self-distillation pre-training phase...")
        print(f"  iterations={sd_cfg.get('num_iterations', 100)}, "
              f"rollout_steps={sd_cfg.get('rollout_steps', 8)}, "
              f"batch_size={sd_cfg.get('batch_size', 16384)}, "
              f"lr={sd_cfg.get('learning_rate', 3e-4)}, "
              f"n_samples={n_samples}")
        print("=" * 80)

        summary = run_self_distillation(
            policy=self.alg.policy,
            env=self.env,
            obs_normalizer=self.obs_normalizer,
            device=self.device,
            num_iterations=sd_cfg.get("num_iterations", 100),
            rollout_steps=sd_cfg.get("rollout_steps", 8),
            batch_size=sd_cfg.get("batch_size", 16384),
            learning_rate=sd_cfg.get("learning_rate", 3e-4),
            n_samples_per_action=n_samples,
            writer=self.writer,
            logger_type=self.cfg.get("logger", "tensorboard"),
        )

        self._self_distill_summary = summary

        if self.alg.ema is not None:
            self.alg.ema.reset_to_current()

        self.alg.optimizer = torch.optim.AdamW(
            self.alg.policy.parameters(),
            lr=self.alg.learning_rate,
            betas=self.cfg.get("algorithm", {}).get("adam_betas", (0.9, 0.999)),
            weight_decay=self.cfg.get("algorithm", {}).get("weight_decay", 1e-4),
        )

        if self.log_dir is not None and not self.disable_logs:
            sd_path = os.path.join(self.log_dir, "self_distill_checkpoint.pt")
            torch.save({
                "model_state_dict": self.alg.policy.state_dict(),
                "obs_norm_state_dict": self.obs_normalizer.state_dict() if self.empirical_normalization else None,
                "self_distill_summary": summary,
            }, sd_path)
            print(f"  Saved self-distillation checkpoint to {sd_path}")

        print("=" * 80)
        print(f"Self-distillation complete. Wall time: {summary['wall_time_sec']:.1f}s")
        print("=" * 80 + "\n")
