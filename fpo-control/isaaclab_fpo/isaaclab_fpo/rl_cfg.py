...
class FpoRslRlPpoAlgorithmCfg:  # Keeping name for backwards compatibility
    ...
    The canonical locomotion baseline (Nov 2025) uses 'fixed'. Motion tracking uses 'adaptive'.
    """
    ...
    desired_kl: float = -1.0
    """The desired KL divergence. Default is -1 (disabled).

    When > 0 and schedule='adaptive', the learning rate is adjusted to keep KL divergence
    near this target. The canonical locomotion baseline (Nov 2025) uses -1 (fixed schedule).
    Motion tracking uses 0.0001 with adaptive schedule.
    """
    ...
    motion tracking uses 0.01. Also used as the SPO epsilon in 'spo' and 'aspo' modes.
    """
    ...
    The canonical locomotion baseline (Nov 2025, run 03yjn5lj) uses 16. Motion tracking
    also uses 16. Gains plateau beyond 16.
    """
...
@configclass
class FpoSelfDistillationCfg:
    """Configuration for optional self-distillation pre-training phase."""

    enabled: bool = False
    """Whether to run self-distillation before FPO training. Default is False."""

    num_iterations: int = 100
    """Number of distillation gradient steps. Default is 100."""

    rollout_steps: int = 8
    """Environment steps per data collection round during distillation. Default is 8."""

    batch_size: int = 16384
    """Mini-batch size for each distillation gradient step. Default is 16384."""

    learning_rate: float = 3e-4
    """Learning rate for the distillation optimizer. Default is 3e-4."""

    n_samples_per_action: int | None = None
    """CFM samples per action for distillation loss. Default is None (uses task default)."""
...
class FpoRslRlOnPolicyRunnerCfg:
    ...
    self_distillation: FpoSelfDistillationCfg = FpoSelfDistillationCfg()
    """Self-distillation pre-training configuration. Disabled by default."""
