...
class WandbSummaryWriter(SummaryWriter):
    ...
    def add_histogram(self, tag, values, global_step=None, bins="tensorflow"):
        # Filter out NaN/inf values to prevent numpy from computing absurd bin counts
        if isinstance(values, np.ndarray):
            values = values[np.isfinite(values)]
        if not hasattr(values, 'size') or values.size == 0:
            return
        try:
            super().add_histogram(tag, values, global_step=global_step, bins=bins)
        except Exception:
            pass
        try:
            wandb.log({self._map_path(tag): wandb.Histogram(values)}, step=global_step)
        except Exception:
            pass
