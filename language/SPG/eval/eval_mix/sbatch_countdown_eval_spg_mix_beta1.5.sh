...
TASKS=("countdown")
GEN_LENGTHS=(256)
SAVE_DIR=/home/haozhe

# Generate checkpoints using a for loop
CKPTS=()
for i in {100..8000..100}; do
  CKPTS+=("checkpoint-$i")
done
  ...
  for gen_length in "${GEN_LENGTHS[@]}"; do
    for ckpt in "${CKPTS[@]}"; do
      ...
      CUDA_VISIBLE_DEVICES=$GPU_LIST torchrun \
        ...
        --output_dir "eval_results/eval_results_countdown_base_spg_mix_beta1.5_weight0.5_elbo0.055_$ckpt" \
        --model_path "GSAI-ML/LLaDA-8B-Instruct" \
        --checkpoint_path "${SAVE_DIR}/spg/countdown_base_spg_mix_beta1.5_weight0.5_elbo0.055/$ckpt"
    done
  done
done
