...
def print_results(setups, directory_name=None):
    """Print aggregated results in table format."""
    if directory_name:
        print(f"\n{'='*80}")
        print(f"Results for: {directory_name}")
        print(f"{'='*80}")
...
if __name__ == "__main__":
    # Parent directory containing all eval result directories
    parent_directory = "/home/haozhe/SPG/eval/eval_mix/eval_results"
    
    # Find all directories in parent directory
    if os.path.isdir(parent_directory):
        subdirs = sorted([d for d in os.listdir(parent_directory) 
                         if os.path.isdir(os.path.join(parent_directory, d))])
        
        print(f"\nProcessing {len(subdirs)} evaluation directories...\n")
        
        for subdir in subdirs:
            subdir_path = os.path.join(parent_directory, subdir)
            setups = aggregate_results(directory=subdir_path)
            
            if setups:  # Only print if there are results
                print_results(setups, directory_name=subdir)
    else:
