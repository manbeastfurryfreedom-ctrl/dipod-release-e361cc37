...
class DiffuGRPOTrainer(GRPOTrainer):
    ...
    def __init__(
        ...
        self.fpo = fpo
        self.elbo_num_samples = elbo_num_samples

    def _get_sequence_elbo(self, model, input_ids, completion_mask, num_samples=8, stored_randomness=None):
        """
        Estimate the evidence lower bound (ELBO) for a batch of sequences using Monte Carlo sampling.
        """
        batch_size, seq_len = input_ids.shape # seq_len: diffusion block size
        completion_len = completion_mask.sum(dim=1).long() # completion_len: unpadded output length
        prompt_len = seq_len - completion_mask.shape[1]# prompt_len: prompt length

        total_elbo_samples = []
        generated_randomness = []
        should_generate_randomness = stored_randomness is None

        for i in range(num_samples):
            masked_inputs = input_ids.clone()

            if should_generate_randomness:
                mask_rates = torch.rand(batch_size, device=input_ids.device)
                num_tokens_to_mask = (mask_rates * completion_len.float()).long()

                masking_matrix = torch.zeros_like(masked_inputs, dtype=torch.bool)
                for j in range(batch_size):
                    indices = torch.randperm(completion_len[j].item(), device=input_ids.device)[:num_tokens_to_mask[j]]
                    masking_matrix[j, prompt_len + indices] = True
                generated_randomness.append((mask_rates, masking_matrix))
            else:
                mask_rates, masking_matrix = stored_randomness[i]
                num_tokens_to_mask = (mask_rates * completion_len.float()).long()
            
            masked_inputs[masking_matrix] = self.args.mask_id

            logits = model(masked_inputs).logits

            log_probs = F.log_softmax(logits, dim=-1)
            del logits
            original_token_log_probs = torch.gather(log_probs, 2, input_ids.unsqueeze(-1)).squeeze(-1)
            
            summed_log_p_theta = (original_token_log_probs * masking_matrix).sum(dim=1)
            
            safe_num_tokens_to_mask = torch.clamp(num_tokens_to_mask.float(), min=1.0)
            l_prime_pi = (completion_len.float() / safe_num_tokens_to_mask) * summed_log_p_theta
            total_elbo_samples.append(l_prime_pi)
            del masked_inputs, log_probs, original_token_log_probs, summed_log_p_theta

        stacked_elbos = torch.stack(total_elbo_samples, dim=1)
        elbos = stacked_elbos.mean(dim=1)

        del total_elbo_samples, stacked_elbos
        torch.cuda.empty_cache()

        return elbos.to(torch.float32), (generated_randomness if should_generate_randomness else None)
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if self.fpo:
            ...
            elbo_randomness = inputs["elbo_randomness"]
            
            input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
            current_elbos, _ = self._get_sequence_elbo(
                model, input_ids, completion_mask, num_samples=self.elbo_num_samples, stored_randomness=elbo_randomness
            )
            
            old_elbos = inputs["old_elbos"]
            
            rho = torch.exp(current_elbos - old_elbos)
            
            advantages = inputs["advantages"]
            loss1 = rho * advantages
            loss2 = torch.clamp(rho, 1 - self.epsilon, 1 + self.epsilon) * advantages
            loss = -torch.min(loss1, loss2).mean()  # Average over the batch
            loss = -torch.min(loss1, loss2).mean()-current_elbos.mean()*self.args.dipod_beta # plus regularizer
            
            if self.beta != 0.0:
                ref_elbos = inputs["ref_elbos"]
                kl_div = (current_elbos - ref_elbos).mean()
    ...
    def _generate_and_score_completions(
        ...
        ref_elbos = None
        mask_seeds = None
        elbo_randomness = None

        if self.fpo:
            with torch.no_grad():
                old_elbos, elbo_randomness = self._get_sequence_elbo(
                    self.model, prompt_completion_ids, completion_mask, num_samples=self.elbo_num_samples
                )
                if self.beta != 0.0:
                    with self.accelerator.unwrap_model(self.model).disable_adapter():
                        # The second call reuses the generated randomness
                        ref_elbos, _ = self._get_sequence_elbo(
                            self.model,
                            prompt_completion_ids,
                            completion_mask,
                            num_samples=self.elbo_num_samples,
                            stored_randomness=elbo_randomness,
                        )
        else:
        ...
        # advantages = 1+torch.sigmoid(advantages)/5 # tricks with advantages
        # advantages += 0.2
        ...
        if 'prompt_completion_ids_expanded' in locals():
            del prompt_completion_ids_expanded
        ...
        return {
            ...
            "elbo_randomness": elbo_randomness,
        }
